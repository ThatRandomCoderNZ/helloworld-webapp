...
export default defineComponent({
  ...
  methods: {
    handleLesson(){
      ...
      if(this.lessonAttempt.toLowerCase() === this.currentLesson.english.toLowerCase()){
        this.lessonAttempt = '';
        this.lessonProgress += 1;
        this.assignRandomLesson();

        let correct = gsap.to(".lesson-input", {
                    scale: 1.05,
                    duration: 0.1,
                    outlineColor: '#0fbf59',
                    yoyo: true,
                    repeat: 1,
                    ease: "power1.out",
                    onComplete: () => {correct.kill()}
                });
      } else {
        this.lessonAttempt = '';
        let incorrect = gsap.to(".lesson-input", {
                    scale: 1.05,
                    duration: 0.1,
                    outlineColor: '#dd7474',
                    yoyo: true,
    ...
    assignRandomLesson(){
      let nextLesson = this.getRandomInt(this.lessonData.length);
  ...
  computed: {
    ...
    englishHint(){
      return this.currentLesson.english.length > 1 ? '[ ' + this.currentLesson.english +' ]' : '[ ' + this.currentLesson.english + ' ]';
    },
  }
});
</script>
  ...
  <div class="reader-modal" v-if="showModal">
    <div class="modal-content">
      ...
      <div class="modal-description">
        <div class="modal-lesson" v-for="lesson in lessonData">
    ...
    <div class="center-content">
      <div class="main-lesson-container">
        ...
        <div class="keyboard-container">
          <input v-model="lessonAttempt" @keyup.enter="handleLesson" class="lesson-input" type="text" ref="userEntry" />
        </div>
      </div>
    </div>
  </div>
